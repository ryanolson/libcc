...
!   if( a.eq.1 .and. b.eq.1 ) write(6,*)'c x3temp ',c,x3temp

      
!if(a .eq. 1 .and. b .eq. 2 .and. c .eq. 3 ) write(6,*)'a b c i j f d1 denom ',a,b,c,i,j,f,d1,denom
      if(a.eq.b) cycle
      ...
      x3temp  = x3temp + f*d1*denom
      x3accum = x3accum + x3temp
!if(a .eq. 1 ) write(6,*)'a b c i j x3',a,b,c,i,j,x3
...
! ets
      ...
      t1(a,k) = t1(a,k) + ( t3_ab4*voe_ij(b,c) + t3_ab6*voe_ji(b,c) )*denom
      t1(b,k) = t1(b,k) + ( t3_ab4*voe_ij(a,c) + t3_ab6*voe_ji(a,c) )*denom*om
!   if( a.eq.1 .and. b.eq.5 ) write(6,*)'c x3 ', c,x3temp
   end do
!   if( a.eq.1 .and. b.eq.1 ) write(6,*)'x3accum ',x3accum
!     if(x3temp.ne.0.d0) write(6,*)'a,b,x3temp ', a, b, x3temp
   end do
   end do
