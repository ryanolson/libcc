...
do iwrk = sr, sr+nr-1
  ...
  if(i.ne.iold) then
    ...
    call tranmd_smp(ve_i,nu,nu,nu,1,23)
    iold = i
  end if

  if(j.ne.jold) then
    if(smp_me.eq.0) call ddcc_t_getve(nu,j,tmp,ve_j)
    call tranmd_smp(ve_j,nu,nu,nu,1,23)
    jold = j
  end if

  if(k.ne.kold) then
    if(smp_me.eq.0) call ddcc_t_getve(nu,k,tmp,ve_k)
    call tranmd_smp(ve_k,nu,nu,nu,1,23)
    kold = k
  end if
...
! moved iij and ijj tuples outside this loop
! ==========================================
! if(k.eq.(j-1)) then
!   call ddcc_t_ijj_big(no,nu,i,j,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
!   call ddcc_t_iij_big(no,nu,i,j,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
! end if
!
! if(k.eq.1) then
!   if(j.eq.2 .and. i.eq.3) then
!     call ddcc_t_ijj_big(no,nu,j,k,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
!     call ddcc_t_iij_big(no,nu,j,k,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
!   end if
!   if(j.eq.(i-1)) then
!     call ddcc_t_ijj_big(no,nu,i,k,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
!     call ddcc_t_iij_big(no,nu,i,k,v1,t2,vm,v3,t3,voe,t1,eh,ep,tmp)
!   end if
! endif
...
! d_vvvo is stored as ve(p4,p5,p6,h?)
! d_vovv is stored as ve(p4,h?,p6,p5)
