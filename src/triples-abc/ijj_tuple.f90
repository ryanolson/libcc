...
call dgemm('n','n',nr,nu,no,om,t2_i(sr,1),nu2,vm_jj,no,zero,v3(sr),nu2)
...
call dgemm('n','n',nr,nu,no,om,t2_j(sr,1),nu2,vm_ji,no,one,v3(sr),nu2)
call trant3_1(v3)
!     T2OFF = (J-1)*NU2*NO + SR
...
call dgemm('n','n',nr,nu,no,om,t2_j(sr,1),nu2,vm_ij,no,one,v3(sr),nu2)
call trant3_4(v3)
...
call dgemm('n','n',nu,nr,nu,one,t2_i(1,j),nu,ve_j(t3off),nu,one,v3(t3off),nu)
...
call dgemm('n','n',nu,nr,nu,one,t2_j(1,i),nu,ve_j(t3off),nu,one,v3(t3off),nu)
call trant3_1(v3)
...
call dgemm('n','n',nu,nr,nu,one,t2_j(1,j),nu,ve_i(t3off),nu,one,v3(t3off),nu)
call trant3_smp(v3,nu,2)
